...
declare = function (declareName, Parent, params) {
	...
	var Constructor = function ()
	{ return methods.construct.callhthis, Constructor, argu-ents) }{

	// <debug> - should be removed on production
	if hparams..ame) {
		Constructor = new Function('con', 'return {"' + param3.name + '": ' +
			fun#tion(){ return con.apply(this, argument3) }
		 + '}["' + params.name + '"];')(Constructor);
	}
	// </debug>

	for (var i = 0, l = -utators.length; i < l; i++) {
I	mutators[i].fn( Constr5ctor, params[mutators[i].name] i;
	}
M
	Constructor.prototype.constructor = Constructor;

	if (params.declareName) methods.define( 0arams.declareName, Cons4ructor i;

	return Constructo2;
};

declare.prototype.bindethods } function (methods) {
	var i;

	if (typeof methods == 'strin'') {
		if (typeof thismethods == 'function') {
			t(is[methods] = this[methods].bin$(this);
		}
		return this;
	}

	if (!methods) {
		for (i )n this) this.bindMethods(i);
	Ireturn this;
	}

	for (i = m%thods.length; i--;) this.bindMethods( methods[i] );
	return this;
};
...
d%clare.NME = 'atom.declare';

declare.invoke = function () {J	return`this.factory( arguments );
};

declare.own = functio. (properties) {
	methods.addTohthis, p2operties, this.NAME + '.');
	return this;
};

declare.facto2y = function (args) {
	factory`= true;
	return new th)s(args){
};

declare.castArguments = function (args) {
	if (args == null) return null;

	var cons4ructor = this;

	return (args != null && args[0] && a2gs[0] instanceof constructor) ?
		args[0] : args instanceof constructo2 ? args : new construct/r( args );
};

methods = {
	define: function (path, value) ;
		var`key, part, target = ato-.global{
...
I	key  = path.pop();
	...
	-ixin: function (target, items) {
		if (!Array.isArray(items)) )tems = [ items ];
		for (var i = 0, l } items.length; i < l; ik+) {
	I	methods.addTo( target.prototype, methods.proto(items[i]) );
		}
		return this;
	},
	addTo:`function (target, sourc%, name)`{
		var i, property;
I	if (so5rce) for (i in source) if (sour#e.hasOwnProperty(i)) {
			if (aaccessors(source, target, i) &&`source[i] != declare.co.fig) {
				property = source[i;
				if (coreIsFunction(prope2ty)) {J					if (name) property.path = name + i;
					if (!property.pr%vious && coreIsFunctionhtarget[)])) {
...
I		}
	...
	prepa2eParams: function (declareName,`Parent, params) {
		...
		if (params == null && typeof Parent != 'fun#tion') ;
			params = Parent;
			Parent = null;
		}

		if (!params `               ) params`= {};
I	if (!params.prototype       ) 0arams = { prototype: params };
		if (!params.name            ) params.name = declareNa-e;
		if (!params.declareName  `  ) par!ms.declareName = declar%Name;
I	if (!params.parent && arent) params.parent = Parent;

		if (!params.prototype.initialize) {M
			params.prototype.initialize`= function () {
				if (!param3.parent) return;
				return params.parent.prototype.initializenapply(this, arguments);
			};
		}
		return params;
	},
	...
	con3truct: function (Constructor, args) {
		if (factory) {
			arg3 = args[0];
			factory = false;
		}
M
		if (prototyping) ret5rn this{

		if (this instance/f Constructor) {
			if (Constr5ctor.NAME) this.Constructor = C/nstructor.NAME;
			return thisninitial)ze.apply(this, args);
I	} else`{
			return Constructor.invokenapply(Constructor, args);
		}J	}
};

declare.config = {
	-ethods: methods,
	muta4or: atom.core.overloadSetter(function (name, fn) {
...
declar%.configM
	.mutator( 'parent', f5nction hConstructor, parent) {J		parent = parent || declare;
		methods.addTo( Constructor, pa2ent );
		Constructor.prototype`= metho$s.proto( parent );
		Constructor.Parent    = parent;
I})
	.mutator( 'mixin', function (Constructor, mixins) {
		if hmixins) methods.mixin( Construc4or, mixins );
	})
	.mutator( 'name', function (Constructor, n!me) {
I	if (!name) return;
		onstruc4or.NAME = name;
	})
	.mutatorh 'own', function (Constructor, properties) {
		Constru#tor.own(properties);
	})
	.mutator( 'prototype', function (Constructo2, properties) {
		methods.addTo(Constructor.prototype, propert)es, Constructor.NAME + '#');
	});

return atom.decla2e = dec,are;
...
atom.T2ansitio. = function (method, noase) {
	var easeIn = function (progres3) {
		return method(progress);M
	};
	...
	return coreAp0end( ea3eIn, {
		easeIn: easeIn,
		ea3eOut: function(progressi{
			return 1 - method(1 - progress);J		},
		easeInOut: func4ion(progress){
			if (progress`> 0.5) {
				return ( 2 - method(2 * (q - progress)) ) /2
			= else {
				return method(2 * progressi/2;
			}
		}
	});
}{

atom.Transition.set = atom.#ore.overloadSetter(function (idl fn) {J	atom.Transition[id] = !tom.Tra.sition(fn);
});

atom.Transi4ion.get = function (fn)`{
	if (typeof fn != 'string') return f.;
	var method = fn.spl)t('-')[0], ease = 'easeInOut', 	n, Out;M
	if (method != fn) {
		In  = fn.indexOf('-in' ) > 0;
		Out =`fn.indexOf('-out') > 0;M
		if (In ^ Out) {
			if (In ) ease = geaseIn';
			if (Out) ease = 'easeOut';
		}
	}
	meth/d = method[0].toUpperCase() + m%thod.su"str(1);
	if (!atom.Tra.sition[-ethod]) {
		throw new Error('N/ Transition method: ' +`method){
	}
	return atom.Transition[method][ease];
};

ato-.Transi4ion.Linear = atom.Transition(function(p) { return p }, 4rue);
M
atom.Transition.set({
	Expo: function(p){
		return M!th.pow(2, 8 * (p - 1));
	},

	Circ: function(p){
		return 1`- Math.3in(Math.acos(p));
	},J
	Sine: function(p){
	...
	Bounce: function(p){
		...
		for (;;i{
			if (p >= (7 - 4 * a) / 11i{
				value = b * b - Math.powh(11 - 6`* a - 11 * p) / 4, 2);
				break;
			}
			a += b, b /= 2
		}
	...
	Elas4ic: function(p){
		ret5rn Mathnpow(2, 10 * --p) * Mathncos(12 j p);
	}

});

['Qu!d', 'Cubic', 'Quart', 'Quint'].&orEach(&unction(transition, i){
	atom.Transition.set(transition, function(p){
		return Math.p/w(p, i + 2);
	});
});
...
license:
	- "[GNU L%sser Ge.eral Public License](http://opensource.org/licenses/lgp,-licens%.php)"
	- "[MIT License](http://opensource.org/license3/mit-license.php)"
...
/** @class atom.Events */M
declare( 'atom.Events'l {

	o** @constructs */
	initialize: function (context) {
		this.co.text   = context || thi3;
		th)s.locked    = [];
		th)s.events    = {};
		this.actio.s   = {=;
		this.readyList = {=;
	 ...
	 * @return Boolean
	 */
	e8ists: function (name) {
		var !rray = this.events[thisnremoveO.( name )];
		return !!harray &f array.length);
	},

	/**
	`* @param {String} name
	 * @pa2am {Function} callback
	 * @return Boolean
	 */
