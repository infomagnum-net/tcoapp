materialAdmin
    ...
    .directive('sparklineBar', function(){
        element_array=[]
        elemens_list=$("#grf-val").html().split(",")
        index_len_element_list=elemens_list.length-1;
        elemens_list_1st=Math.floor(elemens_list[0].split("[")[1])
        elemens_list_nth=Math.floor(elemens_list[index_len_element_list].split("]")[0])
        element_array.push(elemens_list_1st,elemens_list_nth)
        elemens_list.pop(elemens_list[0],elemens_list[index_len_element_list])

        for (i = 0; i < elemens_list.length; i++) { 
            ele_val = element_array.push(Math.floor(elemens_list[i]))
        }
        ...
        return {
            ...
            link: function(scope, element) {
                function sparkLineBar(selector, values, height, barWidth, barColor, barSpacing) {
                 ...
                 element_arr_1=element_array.filter(Boolean)
                sparkLineBar('.stats-bar', element_arr_1, '45px', 3, '#fff', 2);
    ...
    .directive('sparklineLine', function(){
        ...
        return {
            ...
            link: function(scope, element) {
                ...
                element_arr_1=element_array.filter(Boolean)
                sparkLineLine('.stats-line', element_arr_1, 85, 45, '#fff', 'rgba(0,0,0,0)', 1.25, 'rgba(255,255,255,0.4)', 'rgba(255,255,255,0.4)', 'rgba(255,255,255,0.4)', 3, '#fff', 'rgba(255,255,255,0.4)');
