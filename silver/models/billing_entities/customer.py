...
class Customer(BaseBillingEntity):
    ...
    phone_number = models.TextField(max_length=64,null=True, blank=True,
         help_text="mobile no (eg. '9876543210')."
         )
     
    facebook = models.CharField(
        max_length=64, null=True, blank=True,
        help_text="facebook address (eg. 'user@facebook')."
    )                           
    
    twitter = models.CharField(
        max_length=64, null=True, blank=True,
        help_text="twitter address (eg. 'user@twitter')."
    )  

    skype = models.CharField(
        max_length=64, null=True, blank=True,
        help_text="twitter address (eg. 'user@skype')."
    )

    profile_img = models.ImageField(upload_to = 'profile-pics',null=True, blank=True)
