...
from .models.plans import Region,RDS,S3Storage,LoadBalencer

from .models.billing_entities.base import UserArchitectures

from .models.product_codes import Billing_Architecture,Billing_FeatureArchitecture,Billing_ArchitectureType
...
class PlanForm(forms.ModelForm):
    class Meta:
        ...
        fields = ('provider', 'name', 'product_code', 'interval',
                  ...
                  'generate_after','instance_count','metered_features','rds_count','rds_metered_feature','s3_count','s3_metered_features','balancer_count','LoadBalencer_metered', 'enabled', 'private',)
...
class PlanAdmin(ModelAdmin):
    ...
    #                 'trial_period_days', 'enabled', 'private','instance_cost','rds_cost',
    #                 's3_cost','loadbalancer_cost','total_plan_price','product_price','amount']
    ...
    list_display = ['id','name', 'description', 'interval_display',
                    ...
                    'rds_name','balancer_name','s3_name','s3_metered_features']
    ...
    def save_model(self, request, obj, form, change):
        print '***'*20
        print obj.total_plan_price
        print obj.instance_cost
        print obj.rds_cost
        print obj.loadbalancer_cost
        obj.instance_price=obj.instance_cost
        obj.rds_price=obj.rds_cost
        obj.s3_price=obj.s3_cost
        obj.balancer_price=obj.loadbalancer_cost
        obj.amount=obj.total_plan_price
...
class MeteredFeatureAdmin(admin.ModelAdmin):
    #list_display = ('id', 'region','name','unit','price_per_unit','included_units','included_units_during_trial','EBS_volume','Elastic_IP','product_code')   
    list_display = ('id','region','ostype','name', 'unit', 'price_per_unit', 'included_units',
...
class LoadBalencerAdmin(admin.ModelAdmin):
    #list_display = ('id', 'region','name','unit','price_per_unit','included_units','included_units_during_trial','EBS_volume','Elastic_IP','product_code')   
    list_display = ('id','region','ostype','name', 'unit', 'price_per_unit', 'included_units',
...
admin.site.register(Billing_Architecture, ArchitectureAdmin)
admin.site.register(Billing_FeatureArchitecture, FeatureArchitectureAdmin)
admin.site.register(Billing_ArchitectureType, ArchitectureTypeAdmin )
...
site.register(RDS, RDSAdmin)
site.register(S3Storage, S3StorageAdmin)
site.register(UserArchitectures,UserArchitecturesAdmin)
site.register(LoadBalencer,LoadBalencerAdmin)
